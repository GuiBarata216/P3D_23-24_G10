...
public:
	...
	Ray PrimaryRay(const Vector& lens_sample, const Vector& pixel_sample) // DOF
	{
		Vector ray_dir;
		...
		ray_dir = n * focal_ratio * pixel.z + 
			v * (h * focal_ratio * (pixel.y / res_y - 0.5f) - lens_sample.y) + 
			u * (focal_ratio * w * (pixel.x / res_x - 0.5f) - lens_sample.x);
	...
	Vector CameraToWorld(const Vector& vec) {
